...
**Capability packs.** A pack is a named, scoped view of the verb catalog: an allow-list, a hard-denied tier class, a per-tier approval requirement, an optional PII-redaction policy, and optional rate limits (`calls_per_minute` / `calls_per_hour`). Argument caps go further and read the call's own arguments: numeric ceilings per call and per UTC day, a maximum item count, an allow-list of the values a field may take (the shipped `fulfillment-bot` pack pins `status` to the fulfillment transitions), and forbidden keys the call may not carry at all (the same pack refuses any order update that touches `billing`, `customer_id`, `line_items` or `set_paid`). An absent field passes an allow-list, but a present value outside it - or a forbidden key present with any value, null included - denies the call. Packs are bound to identities, not roles alone - a WordPress application password, a specific user or role, or, when WooCommerce is active, a WooCommerce REST API key. A credential with no explicit binding falls back to a safe default pack rather than inheriting full access.
...
vendor/bin/phpunit                            # core suite - framework-agnostic, no WP loaded
vendor/bin/phpunit -c plugin/phpunit.xml.dist # plugin suite - WP shims + adapter stub
